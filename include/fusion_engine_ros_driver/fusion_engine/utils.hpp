...
class Utils {
 public:
  /**
   * @brief Helper method to translate FusionEngine GPSFixMessage to ROS standard GPSFix.
  ...
  static gps_msgs::msg::GPSFix toGPSFix(
    ...
    gps_fix.time = contents.p1_time.seconds +
                   (contents.p1_time.fraction_ns * 1e-9); // time since power-on
  ...
  /**
   * @brief Helper method to translate FusionEngine GPSFixMessage to ROS standard NavSatFix.
   * @param contents Culprit gps data to be translated.
   * @return ROS standard message - NavSatFix.
   */
  static sensor_msgs::msg::NavSatFix toNavSatFix(
      const point_one::fusion_engine::messages::ros::GPSFixMessage& contents) {
    ...
    std::copy(std::begin(contents.position_covariance_m2),
              std::end(contents.position_covariance_m2),
              std::begin(nav_sat_fix.position_covariance));
    nav_sat_fix.position_covariance_type = contents.position_covariance_type;
    return nav_sat_fix;
  }

  /**
   * @brief Helper method to translate FusionEngine IMUMessage to ROS standard Imu.
   * @param contents Culprit gps data to be translated.
  ...
  /**
   * @brief Helper method to translate FusionEngine PoseMessage to ROS standard PoseStamped.
