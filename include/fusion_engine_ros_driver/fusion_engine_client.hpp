...
/*
 * Reads bit stream from the Point One Nav FusionEngine Protocol and notifies 
 * all event listeners attached to this singleton object once a raw 
 * FusionEngine data packet is receieved.
 */
class FusionEngineClient {
    ...
    try {
      while (rclcpp::ok()) {
        ...
        inet_ntop(their_addr.ss_family,
                  get_in_addr((struct sockaddr*)&their_addr), their_ip,
                  sizeof(their_ip));
 ...
 private:
  ...
  int open_tcp_socket() {
    ...
    RCLCPP_INFO(node_->get_logger(), "Opened port %d at ip %s", tcp_port_,
                hostname);
    return 0;
  }
